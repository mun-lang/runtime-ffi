...
namespace mun {
    /** A variant that stores either the successful output of a function invocation or
     * the error state (i.e. callbacks and arguments) necessary to retry.
     */
    ...
    class InvokeResult {
        ...
        using err_type = std::tuple<retry_fn_type, update_fn_type, args_type>;
    ...
    public:
         ...
         */
        explicit InvokeResult(success_type&& success)
            : m_variant(std::move(success)) {}

        /** Constructs a result from a retry callback, its arguments, and an update callback.
         *
        ...
        bool is_ok() noexcept {
            return std::holds_alternative<success_type>(m_variant);
        }
         ...
         *
         * BEWARE: Calling this on a failed invocation result will result in undefined behavior.
         *
         * \return the output of a successful function invocation
         */
        success_type&& unwrap() noexcept {
            return std::move(std::get<0>(m_variant));
        }

        /** Retrieves the error state of a failed function invocation.
         *
         * BEWARE: Calling this on a successful invocation result will result in undefined behavior.
         *
         * \return the error state
         */
        err_type&& unwrap_err() noexcept {
            return std::move(std::get<1>(m_variant));
        }

        /** Retries a failed function invocation and returns the result, or immediately returns on prior success.
         * 
        ...
        /** Keeps retrying the function invocation until success, at which point it returns the function's output.
         *
         * BEWARE: This function will not terminate until the function has been successfully invoked.
         *
    ...
    private:
        ...
        InvokeResult<Output, Args...> retry_impl(std::index_sequence<Is...>) {
            auto err = std::move(unwrap_err());
            auto& retry_fn = std::get<0>(err);
            auto& update_fn = std::get<1>(err);
            auto& args = std::get<2>(err);
            while (!update_fn()) {
                // Spin until an update has occured that might fix the error
            }
            return retry_fn(std::get<Is>(args)...);
        }

        std::variant<success_type, err_type> m_variant;
    };
}
